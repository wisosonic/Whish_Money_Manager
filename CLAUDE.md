...
- **Admin panel → Reports** (user's request, 2026-09-25): `src/components/reports/ReportsSection.jsx`, the first section of `AdminPage` (section `admin-reports`), as WAI-ARIA tabs (arrows mirrored in RTL, Home/End). Only the open report is rendered and fetched. API: `server/reports.js`, `data:export` (like the rest of the panel).
  - **Income** (`IncomeReport.jsx`): the dashboard chart from `GET /admin/reports/income?year=` (SQL monthly sums). The chart body lives in `src/components/reports/IncomeChart.jsx` (with `CHART_SERIES` / `CHART_INK` / tooltip / legend). It's the only place the chart appears: the dashboard's `MonthlyChartModal` was deleted on 2026-09-26 (user's request). `monthlyChartData.js` is split into aggregation and `finalizeMonthlyRows` (labels, rounding, future months → null), so both paths share the same null / rounding rules. A backend test checks the server sums equal `buildMonthlyChartData`.
...
- **Code splitting** (2026-09-25; the bundle had grown to 936 kB, over Vite's 500 kB warning):
  - **Chart:** Recharts is imported only by `src/components/reports/IncomeChart.jsx`, which `IncomeReport` loads with `lazy(() => import(…))` behind `<Suspense fallback={<ChartFallback />}>`. (The dashboard's lazily-loaded chart window is gone.)
  ...
  - **Keep it:** don't import `IncomeChart` statically anywhere (`codebase.test.js` fails if you do; it also fails if `MonthlyChartModal` comes back). `ui/chart.jsx` (shadcn) also imports Recharts but nothing uses it.
...
- **Where transactions come from** (user's statement, 2026-09-25):
  - **Main input:** the CSV statement generated by **Whish Money's API**. Its rows are **real money transactions**, and uploading it is how data normally gets in.
  - **The CSV is the reference:** never let the app round, adjust or "correct" what the file says. Its figures must reconcile exactly (see the reconciliation rule below).
  - **Manual Cash In / Cash Out is deliberately limited:** it exists only to add rows the parser didn't detect. So a manual row is a symptom of a parser gap. Fix the parser (with a synthetic test for the missed format) rather than making manual entry richer.
  - **Priorities:**
    - The CSV engine and import reconciliation come first.
...
- **Monthly chart** (`src/components/reports/IncomeChart.jsx`, data in `src/lib/monthlyChartData.js`; shown in Admin panel → Reports → Income only — the dashboard's "الرسم البياني" button and window were removed on 2026-09-26, user's request). Its tests (`IncomeChart.test.jsx`, the dark-palette test in `theme.test.jsx`) feed it transactions through `tests/frontend/chartHarness.jsx`:
